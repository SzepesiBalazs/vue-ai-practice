...
export const a11yRules = [
    {
        ...
        wcag: "1.4.3",
        selector: "p, span, li, td, th, label, a, h1, h2, h3, h4, h5, h6, button, legend",
        test(el) {
            const style = globalThis.getComputedStyle(el);
    ...
    },
    {
        ...
        test(el) {
            ...
            if (!role || !requiredAttrs[role])
                return null;
            const missing = requiredAttrs[role].filter((attr) => !el.hasAttribute(attr));
            if (missing.length) {
    ...
    },
    {
        ...
        category: "landmark",
        severity: "warning",
        description: "Multiple <nav> elements should be distinguished with aria-label",
        wcag: "2.4.1",
        selector: "nav",
        test(el, root) {
