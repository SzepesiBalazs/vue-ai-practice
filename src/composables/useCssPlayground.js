...
export function useCssPlayground() {
    ...
    const cssSource = ref("");
    const activePreset = ref(flexboxPreset);
    const activeBreakpoint = ref(breakpointOptions.find((b) => b.name === "full") ??
...
<body style="margin:0;padding:0;">
  ${htmlSource.value}
    ...
    const previewWidth = computed(() => {
        if (activeBreakpoint.value.width === 0)
            return "100%";
    ...
    function updateCssProperty(property, value) {
        const regex = new RegExp(String.raw `(${escapeRegExp(property)}\s*:\s*)([^;]+)(;)`);
