...
function getFixForRule(ruleId) {
    const fixes = {
        "contrast-normal-text": "Increase text/background contrast. Tool: https://webaim.org/resources/contrastchecker/",
        "contrast-large-text": "Large text needs 3:1 minimum contrast. Darken text or lighten background.",
        "input-missing-label": 'Wrap the input in a <label>, or use <label for="...">, or add aria-label.',
        "label-empty": "Add visible text inside the <label> element.",
        "img-missing-alt": 'Add alt="" for decorative images, or a descriptive alt for meaningful ones.',
        "img-alt-filename": "Replace filename-like alt text with a content description.",
        "aria-role-button-label": 'Add visible text content or aria-label="...".',
        "aria-hidden-focusable": 'Remove aria-hidden from focusable content, or set descendants to tabindex="-1".',
        "aria-required-attr": "Add the missing required ARIA attribute(s) for the role.",
        ...
        "nav-missing-label": "Add aria-label to each <nav> when multiple navs exist.",
...
export function useA11yChecker() {
    ...
    const filteredIssues = computed(() => {
        ...
        if (activeFilter.value === "all")
            return issues;
        return issues.filter((i) => i.severity === activeFilter.value || i.category === activeFilter.value);
    });
    const summary = computed(() => ({
        total: result.value?.issues.length ?? 0,
        errors: result.value?.issues.filter((i) => i.severity === "error").length ?? 0,
        warnings: result.value?.issues.filter((i) => i.severity === "warning").length ?? 0,
        notices: result.value?.issues.filter((i) => i.severity === "notice").length ?? 0,
    }));
    function scan(root) {
        ...
        for (const rule of a11yRules) {
            const elements = Array.from(root.querySelectorAll(rule.selector));
