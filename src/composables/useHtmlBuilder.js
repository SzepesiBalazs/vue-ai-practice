...
export function useHtmlBuilder() {
    ...
    // --- A11y audit ---
    const a11yIssues = computed(() => auditTree(nodes.value));
    // --- Serialized HTML output ---
    const htmlOutput = computed(() => serializeNodes(nodes.value, 0));
    ...
    function dropElement(tag, parentId) {
    ...
    function updateAttribute(nodeId, key, value) {
    ...
    // --- Update text content ---
    function updateTextContent(nodeId, text) {
        const node = findNode(nodes.value, nodeId);
        if (node)
    ...
    function moveInList(list, id, direction) {
        ...
        for (const node of list) {
            if (moveInList(node.children, id, direction))
                return true;
        }
        return false;
    }
    ...
    // --- HTML serialization ---
    function serializeNodes(list, depth) {
