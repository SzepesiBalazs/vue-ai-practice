...
import { usePhpRunner } from "@/composables/usePhpRunner";
...
export default defineComponent({
  ...
  props: {
    problem: { type: Object as PropType<PhpProblem>, required: true },
  },
  setup(props) {
    ...
    const {
      ...
      runLocal,
      reset,
    } = usePhpRunner(props.problem);

    watch(
      () => props.problem,
      (p) => {
        code.value = p.starterCode;
    ...
    function handleReveal() {
      ...
      if (props.problem.expectedOutput.length > 0) {
        runLocal(props.problem.solutionCode);
      }
    }
