<template>
  <div class="flex flex-col gap-4">
    ...
    <div class="flex flex-col gap-1">
      ...
      <TsOutputConsole
        :lines="output"
        :transpileError="runResult?.transpileError"
      />
    </div>
...
import TsOutputConsole from "@/components/TsOutputConsole.vue";

export default defineComponent({
  components: { CodeEditor, TsOutputConsole },
  props: {
    problem: { type: Object as PropType<TsProblem>, required: true },
  },
  setup(props) {
    const code = ref(props.problem.starterCode);
    const revealed = ref(false);

    let runner = useTsRunner(props.problem);
    ...
    function handleRun() {
      runner = useTsRunner(props.problem);
